...
def main():
    ...
    print(f"🛡️  DEFENSE ENABLED: {config.DEFENSE_TYPE} (threshold=median×{config.NORM_THRESHOLD_MULTIPLIER})")
    print("="*60 + "\n")
    ...
    if 'defense_summary' in results:
        ...
        print(f"{'='*60}")
        print(f"Total rounds: {defense_summary['num_rounds']}")
        print(f"Total clients processed: {defense_summary['total_clients_processed']}")
        print(f"Total rejected: {defense_summary['total_rejected']}")
        print(f"Rejection rate: {defense_summary['rejection_rate']*100:.2f}%")
        print(f"Average precision: {defense_summary['average_precision']*100:.2f}%")
        print(f"Average recall: {defense_summary['average_recall']*100:.2f}%")
        print(f"Perfect detection rounds: {defense_summary['perfect_detection_rounds']}/{defense_summary['num_rounds']}")
