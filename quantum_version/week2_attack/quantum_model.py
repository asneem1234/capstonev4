...
class QuantumCircuit:
    ...
    - Variational layers with RY rotations and CNOT entanglement
    """
    ...
    def forward(self, inputs, weights):
        ...
        for i in range(batch_size):
            ...
            # result is a list/tuple of tensors - stack them to keep gradient
            if isinstance(result, (list, tuple)):
                # Stack tensors without calling .item() to preserve gradients
                stacked = torch.stack([r if torch.is_tensor(r) else torch.tensor(r) for r in result])
                outputs.append(stacked)
            else:
                outputs.append(result)
        
        # Stack all batch results into [batch_size, n_qubits] and ensure float32
...
class HybridQuantumNet(nn.Module):
    ...
    2. Quantum processing: 16 → n_qubits → n_qubits quantum features
    3. Classical output: n_qubits → 10 classes
    """
...
def create_model(n_qubits=4, n_layers=4):
    """Factory function to create quantum model"""
    return HybridQuantumNet(n_qubits=n_qubits, n_layers=n_layers)
